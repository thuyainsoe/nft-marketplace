import { PurpleArrow } from "../../../components/Icons";
...
const MoreFromArtist = () => {
  return (
    <div className="w-full main-container py-[40px] md:py-[60px] lg:py-[80px]">
      <div className="px-0 md:px-[25px] lg:px-[50px] w-full flex flex-col gap-[30px] md:gap-[40px] lg:gap-[60px]">
        {/* top section */}
        <div className="w-full flex items-center justify-between">
          <div className="flex flex-col items-start gap-[10px]">
            <h3 className="text-h4 md:text-h3 font-semibold leading-[120%] text-nowrap">
              More from this artist
            </h3>
          </div>
          <button className="hidden md:flex items-center gap-[10px] border border-cta rounded-[20px] h-[60px] px-[30px] w-full justify-center md:w-fit">
            <PurpleArrow />
            <span className="font-semibold text-base">Go To Artist Page</span>
          </button>
        </div>
        {/* bottom section */}
        <div className="w-full grid sm:grid-cols-2 lg:grid-cols-3 gap-[20px] lg:gap-[30px]">
          {items.map((item) => (
            <div key={item.id} className="w-full rounded-[20px] bg-bgsecondary">
